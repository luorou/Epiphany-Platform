...
fn auth_router() -> Router {
    Router::new()
        .route("/get_member_info", post(get_member_info))
        .layer(from_extractor::<auth_member::AuthMember>())
}

fn un_auth_router() -> Router {
    Router::new().route("/get_self_member_info", post(get_member_info))
}

/**
 * controller  get all adminer  
*/
async fn get_member_info(headers: HeaderMap) -> impl IntoResponse {
    ...
    let resp = service::member_service::get_member_by_id(hid_result.unwrap()).await;
    (StatusCode::OK, Json(WarpResponse::build(resp)))
}
/**
 * create adminer
 */
async fn get_s_member_info() -> impl IntoResponse {
    ()
}
