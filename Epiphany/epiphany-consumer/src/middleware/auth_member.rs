    ...
    B: Send,
{
    ...
    async fn from_request(req: &mut RequestParts<B>) -> Result<Self, Self::Rejection> {
        ...
        let member_id = jwt_claims.unwrap().id;
        let cache_token = redis_service::get_string(member_id.to_string().as_str());
        ...
        req.headers_mut().insert("member_id",  HeaderValue::from(member_id));
        Result::Ok(Self {})
    }
}
