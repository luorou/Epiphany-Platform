...
use crate::domain::req::login::{LoginByEmailReq,LoginByPhoneReq};
 ...
 *
 */
pub async fn login_by_email(payload: LoginByEmailReq) -> Result<String, String> {
    let conn = APP_CONTEXT.get::<DatabaseConnection>();
    let db_result = tab_member::Entity::find()
        .filter(tab_member::Column::Email.eq(payload.email))
        .one(conn)
        .await;
    if db_result.is_err() {
...
pub async fn login_by_phone(payload: LoginByPhoneReq) -> Result<String, Option<String>> {
    let conn = APP_CONTEXT.get::<DatabaseConnection>();
    let db_result = tab_member::Entity::find()
        .filter(tab_member::Column::Phone.eq(payload.phone))
        .one(conn)
        .await;
    if db_result.is_err() {
